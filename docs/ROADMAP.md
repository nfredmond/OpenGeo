...
- [x] Public PMTiles dashboard MVP foundation: one project-scoped PMTiles map widget plus constrained feature-count chart widgets through the existing public share path. *(`20260425120000_project_dashboards.sql`, `20260425123000_dashboard_widgets.sql`, `/api/projects/[slug]/dashboard`, `/api/share/[token]/dashboard`, `/p/[token]`.)* Generic dashboard layout editing and cross-filtering remain deferred.
- [ ] Dashboard builder: configurable map + chart widgets with cross-filtering (Vega-Lite).
...
**Exit criteria:** A consultant can onboard a client, share a project with three collaborators, publish a public PMTiles dashboard, and diff two flights of the same site. *(Onboard + share + vector diff shipped 2026-04-18; PMTiles publishing path and the narrow public dashboard foundation are now in place; generic dashboard editing remains.)*
